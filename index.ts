...
const URL: string = "https://instablog9ja.com/category/news-updates/";
// const URL: string = "https://www.lindaikejisblog.com/";
...
const linadIkejiBlog = async () => {
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      return data
      .map((props)=>({
        title: props.querySelector("a")?.textContent,
        img: props.querySelector("img")?.getAttribute("src"),
        text: props.querySelector("p.story_description")?.textContent,
        date: split(props.querySelector("div.post_age")?.textContent),
    ...
    console.log("done gofundme scrapping");
  }
};
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      .map((props)=>({
        title: props.querySelector("a")?.textContent,
        img: props.querySelector("img")?.getAttribute("src"),
        text: props.querySelector("p")?.textContent,
        date:(props.querySelector("time.entry-date")?.textContent),
        url: props.querySelector("a")?.getAttribute("href"),
      }))
    });
...
//body > div.main > div.board > div > div > div.col-md-9.main_board > div > div.col-md-12 > div:nth-child(1) > article > figure > h1 > a